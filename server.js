...
function serveFile(res, pathname){
  if(pathname === "/coladoki" || pathname === "/coladoki/"){
    res.writeHead(302, {"Location":"/coladoki.html"});
    res.end();
    return;
  }
  if(pathname === "/jag" || pathname === "/jag/" || pathname === "/jag.html" || pathname === "/gorai.html" || pathname === "/gourai.html" || pathname === "/gorai_complete_test.html"){
    res.writeHead(302, {"Location":"/coladoki.html"});
    res.end();
    return;
  }
...
const server = http.createServer(async (req, res)=>{
  ...
  try{
    ...
    if(url.pathname === "/api/sessions/start" && req.method === "POST"){
      ...
      const iframeUrlFor = sessionId=>`/coladoki.html?machine=${encodeURIComponent(machine.machineId)}&server=${encodeURIComponent(url.origin)}&creditBaseline=0&playSessionId=${encodeURIComponent(String(sessionId || ""))}`;
...
server.listen(PORT, ()=>{
  console.log("COLADOKI operations server");
