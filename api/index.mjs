...
export default async function handler(req, res){
  try{
    ...
    if(pathname === "/api/sessions/start" && req.method === "POST"){
      ...
      const iframeUrlFor = sessionId=>"/coladoki.html?machine=" + encodeURIComponent(machine.machineId) + "&server=" + encodeURIComponent(origin) + "&creditBaseline=0&playSessionId=" + encodeURIComponent(String(sessionId || ""));
