...
通常時は8種類のモードで管理するゲーム数テーブルと小役直撃からボーナスを抽選します。初代「沖ドキ！」の公開解析値を基準にし、設定1～6の機械割97.0% / 99.7% / 101.7% / 103.9% / 106.2% / 107.9%に合わせて通常時の非公開部分を補正しています。補正後の通常ベースは50ptあたり約24.3～24.8Gです。BIGは70G AT・純増平均210pt、REGは30G AT・純増平均90ptで、消化中は1G連ストックを抽選します。AT中のベル／リプレイナビは6通りの押し順をランダム指定し、押し順ミス時は役不成立・2pt払い出しとなります。天国・コラドキ・超コラドキ・保障モードは32G以内の連チャン対象です。旧 `/jag` ルートは互換用に `/coladoki` へ転送します。
